<script lang="ts">
  ...
  onMount(() => {
    ...
    displayedColors = shuffledkeysArray
      .slice(0, initSize)
  ...
  async function handleScroll(): Promise<void> {
    ...
    if (scrollHeight - scrollTop <= offsetHeight + 100) {
      ...
      if (nextColors.length < loadMoreSize) {
        nextColors = [
          ...nextColors,
          ...shuffledkeysArray.slice(0, loadMoreSize - nextColors.length),
        ];
      }
      const nextColorsIndexed: string[][] = nextColors.map((colorname, i) => [
        i.toString(),
        colorname,
      ]);
  ...
  const meta = {
    ...
    description:
      "Explore a vast collection of cheeky color swatches to find the perfect shade for your design projects",
...
{#if isInitializing}
  <span
    class="bg-gradient-to-r from-purple-600 via-pink-500 to-red-500 bg-clip-text font-bold text-transparent"
    >Loading...</span
  >
{/if}

<div class="colorcontainer">
  <div class="colors flex h-screen flex-wrap justify-center">
    {#each displayedColors as [, colorname]}
      <div class="m-1 h-48 w-48">
        <ColorBox
          {colorname}
