    ...
    <Modal bind:showModal={showModal}>
        ...
        <div class="modal-section">
            ...
            <div class="mode-grid">
                <div class="mode-card">
                    ...
                    <img src="/huehunter-assets/color-mix_RGBmix.webp" alt="RGB color mix" class="mode-image" />
                </div>
                <div class="mode-card">
                    <h4 class="mode-title">
                    ...
                    <img src="/huehunter-assets/color-mix_CMYKmix.webp" alt="CMYK color mix" class="mode-image" />
                </div>
            </div>
        </div>
        ...
        <div class="modal-footer">
            ...
            <div class="modal-social">
                ...
                <a href="https://github.com/organisciak" aria-label="Follow me on Github" target="_blank" class="social-icon">
                    <Fa icon={faGithub} />
                </a>
                <a
            ...
            {#if playMode === 'PRACTICE'}
                <div class="mode-toggle">
                    ...
                    {#each ['RGB', 'CMYK'] as mode}
                        ...
                        {:else}
                            <a class="mode-link" data-sveltekit-prefetch href="{playMode.toLowerCase()}-{mode.toLowerCase()}">
                                {mode}
                            </a>
                        {/if}
                    {/each}
            ...
            <div class="panel main-panel">
                ...
                {#if startMenu}
                    <div class="instructions">
                        <p class="panel-body">
                            You're given a <span class="highlight">target</span> color, and you have to guess what composition of
                            {#if colorMode==='RGB' }
                                <span class="tone red"> red</span>, <span class="tone green">green</span>, and <span class="tone blue">blue</span>
                        ...
                        <p class="panel-body centered">
                            {#if colorMode==='RGB' }
                                <img src="/huehunter-assets/color-mix_RGBmix.webp" alt="RGB color mix" class="instruction-image" />
                            {:else if colorMode==='CMYK' }
                                <img src="/huehunter-assets/color-mix_CMYKmix.webp" alt="CMYK color mix" class="instruction-image" />
                            {/if}
                        </p>

                        <p class="panel-body">
                            Get as close as you can. Each day has <span class="highlight">5</span> target colors to get as high a score as possible.
                        </p>
                        <p class="panel-body">
                            Before you start, try <a data-sveltekit-prefetch class="inline-link" href="practice-{colorMode.toLowerCase()}">Practice Mode</a>.
                            There's also
                            {#if colorMode==='RGB' }
                        ...
                        <p class="panel-note">
                            ...
                            {:else if colorMode==='CMYK' }
                                CMYK mode is based on inks, and is subtractive: it gets darker as you add color. Black is redundant.
                            {/if}
                        </p>
                    </div>
                    <div class="panel-actions">
                        <button class="guesser-button-lg" type="button" on:click={() => startedDaily[colorMode] = true}>Start</button>
                        <a
                ...
                {:else}
                    <div class="target-panel">
                        {#if practiceLock }
                            <p class="panel-note">Practice is disabled while a daily game is going on. Try it later!</p>
                        {:else }
                            {#if playMode === 'DAILY'}
                ...
                {#if finished || (playMode === 'INFINITE' && attempts > 0) }
                    <div class="score-block">
                        ...
                        {#if playMode == 'DAILY' && shareable}
                            <button class="guesser-button-sm" type="button" on:click={() => share(dayScore/attempts)}>
                        ...
                        {#if calculateBoundScore(dayScore/attempts, rawScoreThreshold, 10) < 4.5 }
                            ...
                            <a class="guesser-button-sm secondary button-link" data-sveltekit-prefetch href="practice-{colorMode.toLowerCase()}">
                                Practice
                            </a>
                        {/if}
                    </div>
                ...
                <div class="mode-message">
                    {#if playMode === 'PRACTICE'}
                        {#if finishedDaily['RGB']}
                            <p class="panel-body">
                                Play <a data-sveltekit-prefetch class="inline-link" href="infinite-rgb">Infinite RGB</a>
                            </p>
                        {/if}
                        {#if (colorMode == 'CMYK') && finishedDaily['CMYK']}
                            <p class="panel-body">
                                Or <a data-sveltekit-prefetch class="inline-link" href="infinite-cmyk">Infinite CMYK</a>
                            </p>
                        {/if}
                    {:else if playMode == 'DAILY' && finished}
                        <p class="panel-body">
                            Challenge yourself again tomorrow, or try
                            <a data-sveltekit-prefetch class="inline-link" href="infinite-{colorMode.toLowerCase()}">Infinite Mode</a>
                        </p>

                        {#if colorMode == 'RGB' && !finishedDaily['CMYK']}
                            <p class="panel-body">
                                Want a challenge? Play <a data-sveltekit-prefetch class="inline-link" href="daily-cmyk">CMYK Daily Mode</a>
                            </p>
                        {:else if colorMode == 'CMYK' && !finishedDaily['RGB']}
                            <p class="panel-body">
                                You finished CMYK. Try the RGB <a data-sveltekit-prefetch class="inline-link" href="daily-rgb">Daily Mode</a>
                            </p>
                        {/if}
                    {/if}
                </div>
        ...
        {#if !finished && !startMenu}
            <div class="panel controls-panel">
                    ...
                    <div class="slider-grid">
                        {#each Object.entries(colorMode === 'RGB' ? rgbColors : cmykColors) as [color, value]}
                            <label class="slider-row">
                                ...
                                />
                                <span
                                    style="{textShadowCSS(color, value)}; color:{colorMode === 'RGB' ? rgbToHexByKey(value, color) : cmykToHexByKey(value, color)}"
                                    class="slider-value"
                                >
                                    {value}
                                </span>
                ...
                <div class="controls-actions">
                    ...
                    {:else}
                        <button class="guesser-button-lg" type="button" on:click={getNextColor}>Next Practice Color</button>
                        {#if !finishedDaily['RGB']}
                            <a class="guesser-button-lg secondary button-link" data-sveltekit-prefetch href="/huehunter/daily-rgb">
                                Start Daily{colorMode == 'CMYK' ? ' RGB' : ''} Game
                            </a>
                        {/if}
                        {#if (colorMode == 'CMYK') && (!finishedDaily['CMYK']) }
                            <a class="guesser-button-lg secondary button-link" data-sveltekit-prefetch href="/huehunter/daily-cmyk">
                ...
                <div class="history-list">
                    {#each filteredGuesses.reverse().slice(0,25) as guess }
                        <div class="history-row">
                            <div class="history-swatch" style="background-color:{rgbToHex(guess.targetColor)}"></div>
                            <div class="history-swatch" style="background-color:{rgbToHex(guess.guessColor)}"></div>
...
<style lang='postcss'>
    ...
    .icon-button {
        ...
        background: #14142b;
        color: var(--snes-text);
        box-shadow: 0 4px 0 #05050f, 0 0 0 2px #0a0a1a;
        cursor: pointer;
    ...
    .icon-button:hover {
        ...
        box-shadow: 0 6px 0 #05050f, 0 0 0 2px #0a0a1a;
    }

    .icon-button:active {
        transform: translateY(2px);
        box-shadow: 0 2px 0 #05050f, 0 0 0 2px #0a0a1a;
    }
    ...
    .tone.red { color: var(--snes-red); }
    .tone.green { color: var(--snes-green); }
    .tone.blue { color: var(--snes-blue); }
    .tone.cyan { color: var(--snes-cyan); }
    .tone.magenta { color: var(--snes-magenta); }
    .tone.yellow { color: var(--snes-yellow); }
    .tone.black { color: #f5f5f5; }
    ...
    .guesser-button-sm {
        ...
        color: #1a1a1a;
        box-shadow: 0 4px 0 var(--button-shadow), 0 0 0 2px #0a0a1a;
        cursor: pointer;
        text-decoration: none;
        transition: transform 0.15s ease, box-shadow 0.15s ease;
    }
    ...
    .guesser-button-sm:hover {
        ...
        box-shadow: 0 5px 0 var(--button-shadow), 0 0 0 2px #0a0a1a;
    }

    .guesser-button-lg:active,
    .guesser-button-sm:active {
        transform: translateY(2px);
        box-shadow: 0 2px 0 var(--button-shadow), 0 0 0 2px #0a0a1a;
    }
